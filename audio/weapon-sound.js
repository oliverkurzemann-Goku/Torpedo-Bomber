...
(function(root){
 ...
 root.WeaponSound={fire(ctx,destination,kind){
  ...
  const f=ctx.createBiquadFilter();f.type='bandpass';f.frequency.value=heavy?950:medium?1450:2100;f.Q.value=.65;
  const g=ctx.createGain();g.gain.setValueAtTime(0,t);g.gain.linearRampToValueAtTime(heavy?.78:.58,t+.002);g.gain.exponentialRampToValueAtTime(.001,t+(heavy?.14:.085));
  ...
  bg.gain.setValueAtTime(0,t);bg.gain.linearRampToValueAtTime(heavy?.25:.11,t+.003);bg.gain.exponentialRampToValueAtTime(.001,t+(heavy?.14:.075));

...
const firstWeave=Math.sign(act.aim.x);
e.mode='engage';e.modeT=0;
act=ctx.updateEnemyAir(.05);
assert.equal(act.mode,'engage','fighter must finish a manoeuvre before starting the next');
e.weaveCd=0;e.modeT=0;
act=ctx.updateEnemyAir(.05);
assert.equal(act.mode,'weave','fighter must weave again after recovery');
assert.equal(Math.sign(act.aim.x),-firstWeave,'successive turns must alternate sides');
...
zero.evadeT=0;zero.evadeCd=6;zero.reengage=0;zero.weave=0;
pilot.pos.set(0,400,900);pilot.heading=Math.PI;
const weave=sea.updateZeros(1);
assert(Math.abs(weave.x)>.1,'Zero should weave across a straight pursuit line');

...
Open `../okinawa-preview.html`. This is a separate, complete scenery viewer; it does not change Torpedo Squadron, Thunderbolt Squadron, Remagen, their menus or save files.
...
The caller owns camera, renderer, fog, lights and game state. `viewer.js` is only the standalone inspection UI. No game integration has been performed.

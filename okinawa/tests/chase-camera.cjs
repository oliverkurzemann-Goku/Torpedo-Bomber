...
for(const [store,height]of [[{chaseH:'40'},6],[{tc_chaseHeight_v2:'8'},8],[{tc_chaseHeight_v2:'Infinity'},6],[{tc_chaseHeight_v2:'900'},16]]){
 ...
 const ctx=vm.createContext({THREE,Math,Number,parseFloat,localStorage:{getItem:k=>store[k]??null},camera,P:{pos},killCam:null,dbgOn:false,ST:{FLIGHT:1},state:1,document:{querySelector:()=>null},noseDir:()=>new THREE.Vector3(1,0,0),applyShake(){}});
